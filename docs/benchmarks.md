...
pre-tokenization plus the merge loop — and does **not** include file I/O
...
- **`vocab_size=512` trainer elapsed time:** **4,620.24 ms** (≈ 4.62 s) — full completion, no early-stop. Trainer span only, per the `elapsed_ms` scope note above.
...
- **`vocab_size=32000` command wall clock (outer `time`):** **3:05.09** (= 185.09 s), with `184.02 s user`, `0.89 s sys`, and `99% cpu` — single-threaded pure-Python on one core, no thrashing. The ~170 ms gap above `elapsed_ms` is corpus read + argparse + import + banner render + atomic save + Python startup combined.
